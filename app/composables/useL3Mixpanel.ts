/**
 * Отправка событий лендинга l3 (l3.amayasoft.uz) в Mixpanel.
 *
 * Отдельный composable от l1 и l2 (см. useL1Mixpanel.ts, useL2Mixpanel.ts):
 * то же правило, что и у той пары — каждый лендинг сам решает, какие у него
 * есть события (у l3, как и у l2, нет платёжной воронки, зато есть свой
 * `landing_googleplay_button_tap`, которого нет ни у l1, ни у l2), и все они
 * лишь пишут в один и тот же проект Mixpanel (тот же токен, см.
 ...
 * (`server/api/l3/mixpanel/track.post.ts`), а тот уже подписывает его токеном.
 *
 * Все вызовы — «выстрелил и забыл»: аналитика не должна ни задерживать
 * интерфейс, ни ронять флоу, поэтому ошибки гасятся молча.
 */

export type L3MixpanelEvent
  = | 'landing_opened'
    | 'landing_email_screen'
    | 'landing_password_screen'
    | 'landing_congratulation_screen'
    | 'landing_appstore_button_tap'
    | 'landing_googleplay_button_tap'

const ENDPOINT = '/api/l3/mixpanel/track'
const STORAGE_KEY = 'amaya_l3_distinct_id'
const UTM_STORAGE_KEY = 'amaya_l3_utm'

/** Метки, которые рекламные ссылки приносят в query. Список должен совпадать
 *  с белым списком в server/api/l3/mixpanel/track.post.ts. */
...
function createId() {
  ...
  return `l3-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`
}
