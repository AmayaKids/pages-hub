/**
 * Meta Pixel лендинга l1 (l1.amayasoft.uz).
 *
 * Отдельный composable от l2 (см. useL2MetaPixel.ts): у лендингов разный
 * набор событий (у l1 есть `Purchase`, у l2 — `Lead`, которого у l1 никогда
 * не было), и раньше оба шли через один файл с именем «useMetaPixel»,
 * маршрутизируя события обоих лендингов на `/api/l2/meta-capi/track» — это
 * читалось так, будто l1 зависит от l2. На деле это два равноправных
 * лендинга одного продукта, которые лишь пишут в один и тот же пиксель
 * (тот же `META_PIXEL_ID`, тот же рекламный кабинет).
 *
 * Каждое событие уходит дважды — в браузер (`fbq`) и на сервер
 * (server/api/l1/meta-capi/track.post.ts → Conversions API) с одним и тем же
 ...
 *  токена Conversions API. Тот же пиксель, что и у l2 — оба лендинга
 *  рекламируют один и тот же продукт в одном рекламном кабинете. */
const META_PIXEL_ID = '1335375415064544'

const CAPI_ENDPOINT = '/api/l1/meta-capi/track'
 ...
 * SPA-переходе «/» ↔ «/auth» ↔ «/payment» ↔ «/congratulations» несколько
 * страниц вызывают этот composable заново, а без флага `fbq('init', …)`
 * отработал бы повторно. Сам загрузчик у Meta уже идемпотентен — он выходит
 * по `if (f.fbq) return`.
 */
...
interface FireDualOptions {
   ...
   *  в Meta (см. server/api/l1/meta-capi/track.post.ts). В браузерный `fbq`
  ...
  email?: string
  /** Сумма и код валюты покупки — только для стандартного `Purchase`. Без
   *  этой пары Meta не считает выручку и ROAS, и оптимизация кампании на
   *  покупки работать не будет. */
  value?: number
  currency?: string
}
...
export function useL1MetaPixel() {
  injectPixelLoader()

  /**
   * Стандартное событие Meta (`CompleteRegistration`, `Purchase`,
   * `PageView`, …) — именно такие можно выбрать целью оптимизации кампании
   * в Ads Manager. Имя должно буквально совпадать с таксономией Meta, это
   * не место для своих названий.
   */
