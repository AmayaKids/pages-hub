/**
 * Meta Pixel лендинга l4 (l4.amayasoft.uz).
 *
 * l4 — буквальная копия l1: тот же платный флоу, тот же набор событий
 * (`Purchase` есть, `Lead` — нет, как у l1; ни то ни другое не как у l2/l3).
 * Composable всё равно отдельный от useL1MetaPixel.ts: у каждого лендинга
 * свой серверный роут и свой `eventSourceUrl`, чтобы конверсии были
 * атрибутируемы домену l4, а не потому что события отличаются от l1. Все
 * четыре лендинга (l1–l4) лишь пишут в один и тот же пиксель (тот же
 * `META_PIXEL_ID`, тот же рекламный кабинет).
 *
 * Каждое событие уходит дважды — в браузер (`fbq`) и на сервер
 * (server/api/l4/meta-capi/track.post.ts → Conversions API) с одним и тем же
 ...
 *  токена Conversions API. Тот же пиксель, что и у l1/l2/l3 — все лендинги
 *  рекламируют один и тот же продукт в одном рекламном кабинете. */
const META_PIXEL_ID = '1335375415064544'

const CAPI_ENDPOINT = '/api/l4/meta-capi/track'
...
interface FireDualOptions {
   ...
   *  в Meta (см. server/api/l4/meta-capi/track.post.ts). В браузерный `fbq`
...
export function useL4MetaPixel() {
  injectPixelLoader()
