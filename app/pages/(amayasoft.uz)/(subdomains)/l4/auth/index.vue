...
import checkboxSvg from '~/assets/images/l4/svg/checkbox.svg'
import eyeSvg from '~/assets/images/l4/svg/eye.svg'
import eyeOffSvg from '~/assets/images/l4/svg/eye-off.svg'
...
const { track, identify } = useL4Mixpanel()
 ...
 */
const SCREEN_EVENTS: Partial<Record<Step, L4MixpanelEvent>> = {
...
async function checkEmailExist() {
  ...
  try {
    const response = await $fetch<{ accountExists?: boolean }>(`${L4_ACCOUNTS_API}/signup-via-only-email`, {
      method: 'POST',
      body: {
        email: fields.email.value,
        language: L4_LANGUAGE,
        gameId: L4_GAME_ID,
        experiment: L4_EXPERIMENT,
        country: L4_COUNTRY
      }
    })
...
async function signIn() {
  ...
  try {
    const response = await $fetch<{ AJWT?: string, id?: number }>(`${L4_ACCOUNTS_API}/login`, {
      method: 'POST',
      body: {
        email: fields.email.value,
    ...
    if (response?.id) {
      ...
      saveL4AccountId(String(response.id))
    }
...
async function resetPassword() {
  ...
  try {
    await $fetch(`${L4_ACCOUNTS_API}/resetPasswordWithPassword`, {
      method: 'POST',
      body: {
        email: fields.email.value,
        gameId: L4_GAME_ID,
