 ...
 * уже уводили, лежит в хранилище (см. useL4Payment.ts). Без неё возврат
...
const { trackStandard, trackPageView } = useL4MetaPixel()
 ...
 */
function trackPurchase(status: L4StatusResponse) {
  ...
  if (!claimL4PurchaseTracking(invoiceId.value)) return

  const price = status.price ?? L4_PRICE
  const currency = status.currency ?? L4_CURRENCY

  // track('landing_billing_purchase', {
  //   Price: price,
  //   Currency: currency,
  //   Subscription_type: status.subscriptionType ?? L4_SUBSCRIPTION_TYPE,
...
async function prepareInvoice({ openGateway = false } = {}) {
  ...
  try {
    const invoice = await createL4Invoice(ajwt.value)

    if (isL4AuthFailure(invoice.status)) {
      await restartFunnel()
      return
    }
    ...
    if (invoice.status === 'already_purchased') {
      if (invoice.invoiceId) {
        ...
        saveL4InvoiceId(invoice.invoiceId)
      }
    ...
    if (invoice.ok && invoice.checkoutUrl && invoice.invoiceId) {
      ...
      saveL4InvoiceId(invoice.invoiceId)
      saveL4CheckoutUrl(invoice.checkoutUrl)
      screen.value = 'ready'

      if (openGateway && !wasL4CheckoutOpened(invoice.invoiceId)) {
        markL4CheckoutOpened(invoice.invoiceId)
...
async function pollStatus(attempt = 1, gatewayFailed = false) {
  ...
  const status = await checkL4PaymentStatus(ajwt.value, invoiceId.value)
...
function goToCheckout() {
  ...
  markL4CheckoutOpened(invoiceId.value)
...
onMounted(async () => {
  ...
  invoiceId.value = String(route.query.invoiceId ?? '') || readL4InvoiceId()
  checkoutUrl.value = readL4CheckoutUrl()
  ...
  if (checkoutUrl.value && wasL4CheckoutOpened(invoiceId.value)) {
    screen.value = 'ready'
    return
  }
