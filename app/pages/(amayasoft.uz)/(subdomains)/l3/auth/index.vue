...
import logoSvg from '~/assets/images/l3/svg/logo.svg'
import congratsPopupBgPng from '~/assets/images/l3/png/congrats-popup-bg.png'
import appStoreBadgeSvg from '~/assets/images/l3/svg/appstore-badge.svg'
import googlePlayBadgeSvg from '~/assets/images/l3/svg/googleplay-badge.svg'
import checkboxSvg from '~/assets/images/l3/svg/checkbox.svg'
import stepsArrowMobileSvg from '~/assets/images/l3/svg/steps-arrow-mobile.svg'
import stepsArrowTabletSvg from '~/assets/images/l3/svg/steps-arrow-tablet.svg'
...
/* ------------------------------------------------------------------ *
 * Порт флоу l2/auth/index.vue (сам он — порт sc-giftflow/v1). l3 — тот же
 * бесплатный флоу на отдельном хосте (l3.amayasoft.uz): аккаунт → бесплатный
 * доступ → финальный экран. Логика (шаги, валидация, запросы) не менялась,
 * добавлено только одно: на финальном экране, помимо App Store, теперь есть
 * ещё и кнопка Google Play — своя для l3, ни у l1, ни у l2 её нет.
 *
 * Хост l3.amayasoft.uz не локализован (`HOST_CONFIGS['l3.amayasoft.uz']
 * .locale.localized === false`, см. shared/hostLandings.ts), поэтому все
 * строки — узбекский хардкод, как и в остальном l3 (../index.vue, ../legal).
 *
...
// `var1` занят l2, `var2` — l1 (см. их auth/index.vue). `var3` для l3 —
// предположение по той же схеме нумерации, бэк это значение не подтверждал:
// уточнить и поправить при необходимости.
const EXPERIMENT = 'UA_Cars2_var3'
...
// TODO: заглушка — реальной ссылки на страницу приложения в Google Play нет
// (ни в этом коде, ни в useAppsInfo() она нигде не зарегистрирована), нужно
// подставить настоящую, когда появится listing.
const GOOGLE_PLAY_URL = 'https://play.google.com/store/apps/details?id=com.amayasoft.cars2.ua'
...
const { track, identify } = useL3Mixpanel()
const { trackStandard, trackCustom, trackPageView } = useL3MetaPixel()
 ...
 * свой (`clean-signin`).
 */
const SCREEN_EVENTS: Partial<Record<Step, L3MixpanelEvent>> = {
...
<template>
  <!--
    ...
    планшетное = `md-tablet` (768), как и во всём остальном l3.
      ...
      >
        ...
        <div class="congrats__content">
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            @click="track('landing_googleplay_button_tap'); trackCustom('LandingGoogleplayButtonTap', { email: fields.email.value })"
          >
            <img
