/**
 * Серверное дублирование событий Meta Pixel лендинга l4 через Conversions API.
 *
 * l4 — буквальная копия l1, белый список событий тот же (`Purchase` есть,
 * `Lead` — нет). Отдельный роут от server/api/l1/meta-capi/track.post.ts
 * всё равно нужен — не потому что события отличаются, а чтобы конверсии
 * были атрибутируемы конкретному домену/`eventSourceUrl`, а не смешивались
 * с l1 под одним роутом. Токен и id пикселя при этом общие (тот же рекламный
 * кабинет, см. app/composables/useL4MetaPixel.ts).
 *
 * Дублирует ровно то, что уже шлёт браузерный пиксель — та же связка
 * `event_id` на обеих сторонах нужна, чтобы Meta склеила пару браузер+сервер
 * в одно событие, а не посчитала дважды. Пишется отдельно от браузерного
 * вызова: сервер не должен ждать успеха/неуспеха этого запроса, чтобы не
 * тормозить страницу.
 *
...
/** Тот же id пикселя, что и в useL4MetaPixel.ts — не секрет, дублирование
 *  константы дешевле, чем тащить её через shared/ ради одной строки. */
const META_PIXEL_ID = '1335375415064544'

/** Ровно те события, которые шлёт лендинг l4 (браузерный пиксель + этот
...
export default defineEventHandler(async (event) => {
  ...
  if (!token) {
    console.warn('[l4/meta-capi] NUXT_CARS2_META_CAPI_TOKEN не задан — событие не отправлено:', eventName)
    return null
  }
  ...
  try {
    ...
    if (!response?.events_received) {
      console.warn('[l4/meta-capi] событие не принято:', eventName, response?.messages)
    }
  } catch (error) {
    console.warn('[l4/meta-capi] не удалось отправить событие:', eventName, error)
  }
