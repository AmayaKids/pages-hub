/**
 * Приём событий лендинга l4 (l4.amayasoft.uz) и отправка их в Mixpanel.
 *
 * l4 — буквальная копия l1, белый список событий тот же. Отдельный роут от
 * server/api/l1/mixpanel/track.post.ts всё равно нужен: держать оба лендинга
 * под одним роутом значило бы разрешать событию прийти с любого из двух
 * хостов без возможности отличить, откуда оно реально пришло. Токен при этом
 * общий (тот же проект Mixpanel, см. nuxt.config.ts → cars2MixpanelToken).
 *
...
/** Ровно те события, которые шлёт лендинг l4. Всё остальное отбрасывается. */
const ALLOWED_EVENTS = new Set([
  ...
  // ниже (см. app/pages/(amayasoft.uz)/(subdomains)/l4/payment/index.vue →
  // trackPurchase()).
...
export default defineEventHandler(async (event) => {
  ...
  if (!token) {
    console.warn('[l4/mixpanel] NUXT_CARS2_MIXPANEL_TOKEN не задан — событие не отправлено:', name)
    return null
  }
  ...
  const payload = [{
    ...
    properties: {
      ...
      ...(name === '$identify'
        ...
        // (см. useL4Payment.ts): Mixpanel различает регистр, и `host`/`Host`
  ...
  try {
    ...
    if (response?.status !== 1) {
      console.warn('[l4/mixpanel] событие отклонено:', name, response?.error)
    }
  } catch (error) {
    console.warn('[l4/mixpanel] не удалось отправить событие:', name, error)
  }
