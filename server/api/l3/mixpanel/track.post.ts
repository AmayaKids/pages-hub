/**
 * Приём событий лендинга l3 (l3.amayasoft.uz) и отправка их в Mixpanel.
 *
 * Для l1 и l2 есть свои отдельные роуты (server/api/l1/mixpanel/track.post.ts,
 * server/api/l2/mixpanel/track.post.ts) — общий файл под одним белым списком
 * разрешал бы события чужого лендинга прийти не с того хоста. Токен при этом
 * общий (тот же проект Mixpanel, см. nuxt.config.ts → cars2MixpanelToken) —
 * разделение именно по тому, какие события какой лендинг имеет право слать,
 * а не по токенам.
 *
...
/** Ровно те события, которые шлёт лендинг l3. Всё остальное отбрасывается. */
...
export default defineEventHandler(async (event) => {
  ...
  if (!token) {
    console.warn('[l3/mixpanel] NUXT_CARS2_MIXPANEL_TOKEN не задан — событие не отправлено:', name)
    return null
  }
  ...
  try {
    ...
    if (response?.status !== 1) {
      console.warn('[l3/mixpanel] событие отклонено:', name, response?.error)
    }
  } catch (error) {
    console.warn('[l3/mixpanel] не удалось отправить событие:', name, error)
  }
